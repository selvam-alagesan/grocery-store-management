...
export default function Profile() {
  const {user,loading,checkAuth} = useAdmin();
   ...
   const empPresent = async(_id)=>{
      ...
      try{
        const response = await API.post('/employee/attendence/present',{id: _id});
   ...
   const absentDays = ()=>{
    if (!user?.attendence) return 0;
    const today = new Date();
    const thisMonth = today.getMonth();
    const thisYear = today.getFullYear();

    const absent = user.attendence.filter(a=>{
          ...
          <Row>
            ...
            <Row>
              ...
              <Form.Group as={Col} >
                ...
                <Form.Control value={user?.work || 'Not Assigned'} disabled/>
              </Form.Group>
